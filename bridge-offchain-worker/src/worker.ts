...
const overRunProtectionCallback = (job) => console.log(`Call at ${new Date().toISOString()} were blocked by call started at ${job.currentRun().toISOString()}`);
...
const main = async () => {
  ...
  if (!proverContractAddress) {
    throw new Error("PROVER_CONTRACT_ADDRESS environment variable is not configured or invalid");
  }
  ...
  if (!ccNextRpcUrl) {
    throw new Error("CC_NEXT_RPC_URL environment variable is not configured or invalid");
  }

  if (!ccNextWalletPrivateKey) {
    throw new Error("CC_NEXT_WALLET_PRIVATE_KEY environment variable is not configured or invalid");
  }

  if (!ccNextErc20MintableAddress) {
    throw new Error("CC_NEXT_ERC20_MINTABLE_ADDRESS environment variable is not configured or invalid");
  }
  ...
  const workerJob = new Cron("* * * * *", { context: data, protect: overRunProtectionCallback }, async (self, context) => {
    ...
    if (blockRange.shouldListen) {
      console.log(`Source chain listener is listening from block ${blockRange.startBlock} to ${blockRange.endBlock}`);
      const erc20TransferEvent = erc20Abi.find(abi => abi.name === "Transfer");
      const erc20SourceChainFilter = await sourceChainPublicClient.createEventFilter({
      ...
      for (const log of logs) {
        ...
        if (decodedLog.eventName == 'Transfer') {
          ...
          if (!isAddressEqual(to, "0x0000000000000000000000000000000000000000") && !isAddressEqual(to, "0x0000000000000000000000000000000000000001")) {
            console.log(`Skipping non-burn transfer: tx=${log.transactionHash}, from=${from}, to=${to}, value=${value}`);
            continue;
          }
        }
        ...
        const { request } = await ccNextPublicClient.simulateContract({
          ...
          args: [
            query,
            ccNextWalletClient.account?.address!
          ],
          value: queryCost,
        });
    ...
    if (ccNextBlockRange.shouldListen) {
      ...
      const tokensMinted = viemUscBridgeAbi.find(e => e.type === 'event' && e.name === 'TokensMinted');
