...
const USCBridgeABI = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'contract-abis/USCBridge.json'), 'utf8')
);
const ProverABI = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'contract-abis/prover.json'), 'utf8')
);
...
async function checkIfQueryUsed(contractAddress, proverAddress, queryId) {
  ...
  try {
    ...
    if (used) {
      console.warn(
        `❌ Query ID ${queryId} was already used by ${proverAddress}`
      );
    } else {
      console.log(
        `✅ Query ID ${queryId} has NOT been used by ${proverAddress}`
      );
    }
    return used;
  } catch (err) {
...
async function checkIfQueryIsLocked(contractAddress, proverAddress, queryId) {
  ...
  try {
    const [amount, unlockTime] = await contract.getLockedQuery(
      proverAddress,
      queryId
    );
    ...
    if (isLocked) {
      ...
      console.log(
        `   → Unlock Time (UTC): ${new Date(unlockTime * 1000).toISOString()}`
      );
    } else {
...
function validateResultSegments(segments) {
  if (!segments || segments.length < 8) {
    console.warn(
      `❌ ERC 20 transfers should have at least 8 result segments, got ${segments.length}`
    );
    return false;
  }
  ...
  const fromAddress = ethers.getAddress(
    ethers.zeroPadValue(ethers.hexlify(segments[5].abiBytes), 32).slice(26) // last 20 bytes
  );

  const toAddress = ethers.getAddress(
    ethers.zeroPadValue(ethers.hexlify(segments[6].abiBytes), 32).slice(26)
  );
  ...
  console.log(
    '✅ Segment validation passed. No issues with from addr, to addr, or mint amount.'
  );
  return true;
}
...
async function checkIfQueryUsedByERC20(contractAddress, erc20Address, queryId) {
  ...
  try {
    ...
    if (used) {
      console.warn(
        `❌ Query ID ${queryId} was already used by ERC20 token ${erc20Address}`
      );
    } else {
      console.log(
        `✅ Query ID ${queryId} has NOT been used by ERC20 token ${erc20Address}`
      );
    }
    return used;
  } catch (err) {
...
async function checkErc20AdminPermission(erc20Address, bridgeAddress) {
  ...
  try {
    ...
    if (isAdmin) {
      ...
      console.warn(
        `❌ USCBridge contract does NOT have DEFAULT_ADMIN_ROLE on ERC20!`
      );
    }
  } catch (err) {
    console.warn(
      '⚠️ Could not check DEFAULT_ADMIN_ROLE on ERC20:',
...
async function grantAdminRoleToBridge(erc20Address, bridgeAddress) {
  ...
  try {
    const alreadyHasRole = await erc20.hasRole(
      DEFAULT_ADMIN_ROLE,
      bridgeAddress
    );
    if (alreadyHasRole) {
    ...
    console.log(
      `⏳ Granting DEFAULT_ADMIN_ROLE on mintable to bridge ${bridgeAddress}... TX: ${tx.hash}`
    );
    const receipt = await tx.wait();
